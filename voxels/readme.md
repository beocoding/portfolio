...
This is a custom voxel rendering engine built on top of **Bevy**, the Rust game engine framework. It renders a 3D voxel world (think Minecraft-style blocks) using a heavily optimized pipeline that avoids the naive "one draw call per block" approach. Instead, it uses **greedy meshing**, **instanced rendering**, and **multi-draw indirect** to collapse the entire visible world into a single GPU draw call per frame, with geometry and material data packed tightly into GPU buffers.

To demonstrate the scale of this: a circular dirt platform 1024 voxels in diameter and 32 voxels thick — approximately 26.4 million voxels across ~800 chunks — renders entirely within **one `multi_draw_indirect` call**. Every chunk, every face direction, every greedy-merged quad is batched into a single GPU command. The CPU overhead for issuing that draw is O(1) regardless of world size.
...
This engine collapses the entire world into **one** `multi_draw_indirect` call per frame, regardless of how many chunks or voxels exist. To put that concretely: the 26.4 million voxel circular platform test — ~800 chunks, all face directions, all greedy-merged quads — is rendered in a single GPU command. The GPU reads an array of `DrawIndirectCommand` structs from a buffer and executes them all in sequence without any further CPU involvement. Adding more chunks to the world does not add more draw calls; it only adds more entries to the indirect command buffer.
...
The final `upload()` of that Vec is one `write_buffer` call for the entire frame, covering every chunk simultaneously.
