...
// ── Misc codegen helpers ──────────────────────────────────────────────────────

/// Join boolean expressions with `&&`; empty input yields `true`.
...
pub fn infer_category(ty: &syn::Type) -> FieldCategory {
    ...
    if let Some(inner) = vec_inner(ty) {
        let inner_cat = if type_is_string(inner) { FieldCategory::String }
            else if type_is_fileblob(inner) { FieldCategory::FileBlob }
            else { FieldCategory::Inline };
