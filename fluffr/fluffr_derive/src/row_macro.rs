...
pub fn as_row(input: TokenStream) -> TokenStream {
    ...
    // All Table impls for the Row struct come from the user's own #[derive(Flat)].
    let row_trait      = impl_write_as_registry(&row_meta, &registry_label);

    // ── Registry struct: #[derive(Flat)] handles all serialization impls ──────
    ...
    quote! {
        ...
        // ── Registry: #[derive(Flat)] handles all serialisation/view impls ───
        #[derive(::fluffr::Flat, Default, Clone,)]
...
fn impl_row_ref_and_eq(row_meta: &TableMeta) -> TokenStream2 {
    ...
    //                  (Owned↔View is generated by impl_owned_view_eq in table_macro)
